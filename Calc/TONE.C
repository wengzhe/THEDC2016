...
#ifdef PWM_HORN_SINGLE
void CL_TONE_SetTone2(uint8_t Tone, uint8_t High, uint8_t Low)//1~7,1~3
{
	if (Tone)
		DL_PWM_SetFreq2(FKey*pow(2,(Tones[Tone-1][Low]/12.0)+High-2));
	else
		DL_PWM_SetFreq2(STOP_FREQ);
}

void CL_TONE_DoubleTone2(uint8_t Tone1, uint8_t High1, uint8_t Low1, uint8_t Tone2, uint8_t High2, uint8_t Low2, float percent)//0~1
{
	float F1=FKey*pow(2,(Tones[Tone1-1][Low1]/12.0)+High1-2);
	float F2=FKey*pow(2,(Tones[Tone2-1][Low2]/12.0)+High2-2);
	DL_PWM_SetFreq2(CL_TONE_CalcFreq(F1,F2,percent));
}

void CL_TONE_SetFreq2(float freq)
{
	DL_PWM_SetFreq2(freq);
}
#endif
